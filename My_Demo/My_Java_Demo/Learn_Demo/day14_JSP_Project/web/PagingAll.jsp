...
<div class="container">
    ...
    <div style="float: left">
        <form class="form-inline" action="${pageContext.request.contextPath}/pageAllController" method="post">
            ...
            <div class="form-group">
                ...
                <input type="text" name="birthplace" value="${condition.birthplace[0]}" class="form-control" id="exampleInputName3">
            </div>
            <div class="form-group">
                <label for="exampleInputEmail2">Email</label>
                <input type="email" name="mailbox" value="${condition.mailbox[0]}" class="form-control" id="exampleInputEmail2">
            </div>
    ...
    <form action="${pageContext.request.contextPath}/deleteSeleteController" method="post" id="seleteForm">
        <table border="1px" class="table table-bordered table-hover">
            ...
            <c:forEach items="${paging.studentList}" var="student" varStatus="s">
                <tr>
            ...
            <ul class="pagination">
                ...
                <c:forEach begin="1" end="${paging.pageCount}" var="i">
                    ...
                    <c:if test="${paging.currentPage == i}">
                        <li class="active"><a
                                href="${pageContext.request.contextPath}/pageAllController?currentPage=${i}&pageShowAnCount=5&name=${condition.name[0]}&birthplace=${condition.birthplace[0]}&mailbox=${condition.mailbox[0]}">${i}</a>
                        </li>
                    </c:if>

                    <c:if test="${paging.currentPage != i}">
                        <li>
                            <a href="${pageContext.request.contextPath}/pageAllController?currentPage=${i}&pageShowAnCount=5&name=${condition.name[0]}&birthplace=${condition.birthplace[0]}&mailbox=${condition.mailbox[0]}">${i}</a>
                        </li>
                    </c:if>
                ...
                <li>
                    ...
                    <c:if test="${paging.currentPage + 1 >= paging.pageCount}">
                    <a href="${pageContext.request.contextPath}/pageAllController?currentPage=${paging.pageCount}&pageShowAnCount=5&name=${condition.name[0]}&birthplace=${condition.birthplace[0]}&mailbox=${condition.mailbox[0]}" aria-label="Next">
                    </c:if>

                    <c:if test="${paging.currentPage + 1 < paging.pageCount}">
                        <a href="${pageContext.request.contextPath}/pageAllController?currentPage=${paging.currentPage + 1}&pageShowAnCount=5&name=${condition.name[0]}&birthplace=${condition.birthplace[0]}&mailbox=${condition.mailbox[0]}" aria-label="Next">
                    </c:if>
                ...
                <span style="font-size: 25px;margin-left: 5px;">
                    共${paging.totalCount}条记录，共${paging.pageCount}页
                </span>
